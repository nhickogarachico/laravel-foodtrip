    ...
    <div class="d-flex">
        <nav class="navbar">
            <div>
                ...
                <a href="#profileSideMenu" role="button" data-bs-toggle="offcanvas" role="button" aria-expanded="false">
                    <img src="{{$user->avatar}}" alt="{{$user->username}} avatar" class="small-avatar ms-2 rounded-circle">
                </a>
        ...
        <form class="flex-fill position-relative" action="{{ route('search') }}" method="GET">
            @csrf
            <input id="mobile-search" type="search" name="search" placeholder="Search" aria-label="Search" autocomplete="off">
            <button id="header-search-button" type="submit"><i class="fas fa-search"></i></button>
        </form>
    </div>

    <!-- Off canvas side menu -->
    <div class="offcanvas offcanvas-start" tabindex="-1" id="profileSideMenu" aria-labelledby="offcanvasExampleLabel">
        <div class="offcanvas-header">
            <div>
                <a href="/{{$user->username}}" class="text-decoration-none">
                    <img src="{{$user->avatar}}" alt="sample profile" class="small-avatar ms-2 rounded-circle">
                </a>
                <p class="ms-2 d-inline"><a href="#" class="text-decoration-none">{{$user->username}}</a></p>
            </div>
    ...
    <nav class="navbar fixed-bottom justify-content-center">
        <ul class="navbar-nav flex-row">
            <li class="navbar-item me-5">
                <a class="nav-link text-white position-relative" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" data-bs-auto-close="outside" aria-expanded="false">
